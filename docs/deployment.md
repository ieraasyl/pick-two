...
vp exec wrangler deploy --config dist/pick_two_product/wrangler.json
vp run smoke https://pick-two-product-staging.ieraasyl.workers.dev
```
...
Production uses `pick-two-product-production.ieraasyl.workers.dev` and a separate D1 database.
Configure the same GitHub credentials in the `production` environment, restricted to `main`,
with a required reviewer. The release gate rejects environments without that rule.
...
Before releasing, set `AUTH_EMAIL_FROM` to a sender on a verified Resend domain and test real
registration and password-reset delivery. The workflow rejects the default testing sender.
...
vp run smoke https://pick-two-product-production.ieraasyl.workers.dev
```
